...
app.get('/allow-cors',jsonParser,cors(),(request,response)=>{
  console.log('GET OOOOOO');
    if (!request.body) return response.sendStatus(400);
    let scriptComments=fs.readFileSync('site/script/script.js',"utf8",
    (error,data)=>{
...
app.post('/allow-cors',jsonParser,cors(),(request,response)=>{
  ...
  app.post('/allow-corss',jsonParser,cors(),(request,response)=>{
    console.log('POST OOOOOO');
      if (!request.body) return response.sendStatus(400);
      let scriptComments=fs.readFileSync('site/script/script.js',"utf8",
      (error,data)=>{
  ...
  // ----------------------------BOX comments system------------
  // *******************************************************************************************************************
  const registerUrl=['http://192.168.1.104','https://nasobe.ru','http://localhost'];
  app.use(cors({
    origin:"*",
    method:"POST",
  ...
  app.post("/comments/nasoberu.json", jsonParser,cors(), function (request, response) {
      if(!request.body) return response.sendStatus(400);
      console.log(`${request.body.userName} - ${request.body.message}`+ "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      console.log(`${response.body}`);
      response.send(`${request.body.userName} - ${request.body.message}`);
  });
  app.post("/ok", jsonParser,cors(), function (request, response) {
      if(!request.body) return response.sendStatus(400);
      console.log(`${request.body.userName} - ${request.body.message}`+ "OKKOLOKOKOKKOKOKOKOOKO");
      console.log(`${response.body}`);
      response.send(`${request.body.userName} - ${request.body.message}`);
  });
  function comments(x){
    app.post('site/'+`${id[x]}`+'.json'+'/', jsonParser,cors(), function (request, response) {
  ...
  console.log(importantBag.id+"____IMOPRTANT BBAG id");
      ...
      const putGit='site/json/'+`${importantBag.id}`+'.json';//puth for

      if(request.body.idea.site.pathname!=='/'){
        fs.access("site/json/allDiscus", function(error){
            if (error) {
        ...
        let fileOnPath="site/json/allDiscus/"+`${importantBag.id}`;
        ...
        function createPathOnMessage(){
          ...
          if(`${request.body.idea.site.pathname}`.split("/").length - 1==2){
            ...
            if(`${request.body.idea.site.host}`=='amir248.github.io'){
              ...
              fileOnPathTwo='site/json/allDiscus/'+pathAbracatabruch(`${request.body.idea.id}`+`${githubPathName}`);
            }else{

              console.log('465');
              console.log(`${importantBag.pathname}`.split("/").length - 1);
              fileOnPathTwo='site/json/allDiscus'+pathAbracatabruch(request.body.idea.site.pathname);
              // onGetPath='site/json/allDiscus'+(`${importantBag.pathname}`);
            }
            ...
            console.log('471');
            fileOnPathTwo='site/json/allDiscus/'+`${importantBag.id}`+pathAbracatabruch(request.body.idea.site.pathname);
          }
  ...
  console.log(fileOnPathTwo+" <---fileOnPathTwo 476");
          ...
          //------------------------------------------------------------------
          // const pathOnSitePathname='site/json/allDiscus/'+`${importantBag.id}`+'.json';
    ...
    app.post('/'+`${id[x]}`+'/',jsonParser,cors(),(request,response)=>{
      ...
      // console.log(importantBag.site+'___________7779');
        ...
        function createFolder(){
          fs.access("site/json/allDiscus", function(error){
            if (error) {
              ...
              fs.mkdir('site/json/allDiscus', err => {
                console.log('====================> 620');
                if(err) throw err; // не удалось создать папку
              });
            }else{
        ...
        if(importantBag.pathname!=="/"){
          ...
          if(`${importantBag.pathname}`.split("/").length - 1==2){
            if(`${importantBag.host}`=='amir248.github.io'){
              ...
              console.log(`${importantBag.id}`+`${importantBag.pathname}`+'/threePage.html'+'<------ 665');
                onGetPath='site/json/allDiscus/'+`${importantBag.id}`+`${`${githubPathName}`}`;
            }else{
            ...
            onGetPath='site/json/allDiscus/'+`${importantBag.id}`+(`${importantBag.pathname}`);
          ...
          fs.stat('site/json/'+`${importantBag.id}`+'.json',(err,stats)=>{
            if(err){
              fs.writeFile('site/json/'+`${importantBag.id}`+'.json', '[{"userName":"user"},{"Message","message"},{"date","00:00:00"},{"idea":"oK"}]', function(err){
             ...
             puthScript='site/json/'+`${importantBag.id}`+'.json';
